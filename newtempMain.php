...
<script>
 ...
 function userDefinedHighlightSelectedNodes()
 {
	...
	for(var i=0;i<selectedNodes.length;i++)
	{
		...
		newoptn.text = elements[selectedNodes[i]].name;
 ...
 function createTheGraphContents()
 {
	...
	loadingArea.innerHTML = "<center>&nbsp; Loading Regulons  &nbsp;</center>";
	...
	<?
		...
		else
		{
			...
			{
				//$arrayOfElements[] = $row[0];	// array to be used to populate the regulon - regulon linkage network
			?>
				var obj = new Object();
				obj.id = <?echo $row[0]?>;
				obj.name = "RegID" + <?echo $row[0]?>;
				obj.description = "Regulon ID : " + <?echo $row[0]?>;
				obj.contentCount = <?echo $row[1]?>;
				theElements.push(obj); // push everything into the list of elements. Delete the ones based on the content count later to avoid migrating to another page (pushing a variable from javascript to php)
			<?
			}
		}
	...
	loadingArea.innerHTML = "<center>&nbsp; Loading Regulon-Regulon Network  &nbsp;</center>";
	...
	<?	
		...
		for($i=0;$i<count($arrayOfElements);$i++)
		{
			...
			$query =  'SELECT Reg_2 FROM `achvas`.`regulon_table_subset2` where Reg_1='. $arrayOfElements[$i] .';';
			...
			$query = 'SELECT Reg_1 FROM `achvas`.`regulon_table_subset2` where Reg_2='. $arrayOfElements[$i] .';';
			$result = mysql_query($query);
			if (!$result) 
			{
				die('Invalid query: ' . mysql_error());
			...
			$listOfNeighbors[] =  $neighborsList;
		}
		
		// listOfNeighbors is a 2D array and it is the adjacencyList. (but has duplicate elements in each array)
		
		for($i=0;$i<count($listOfNeighbors);$i++)
		{
			removeDuplicates($listOfNeighbors[$i]);
		}
		...
		// Now the duplicates are removed. the listOfNeighbors array now is the adjacency list.
		
		for($i=0;$i<count($listOfNeighbors);$i++)
		{
		?> var neighborsList;<?
			for($j=0;$j<count($listOfNeighbors[$i]);$j++)
			{
				?>
				neighborsList.push(<? echo $listOfNeighbors[$i][$j] ?>);
				<?
			}
		?> adjacencyList.push(neighborsList);<?
		}
		?>
			
		
		// converting the ids to their index in theElements array
		/*for(var j=0;j<<? count($listOfNeighbors) ?>;j++)
		{
			for(var k=0;k<<;k++)
			{
				if(neighborsList[j] == theElements[k].id)
				{
					neighborsList[j] = k;
					break;
				}
	...
	/*for(var i=0;i<theElements.length;i++)
	{
		if(theElements[i].contentCount < contentCountThresholdValue)
		{
			theElements.splice(i,1);
			//adjacencyList.splice(i,1); should deal with this in a while
			i--;
		}
	}*/
	
	//loadingArea.innerHTML = "<center>&nbsp; Initializing...   &nbsp;</center>";
	...
	drawTheChart(canvasID,selectionCanvasID,levelRanges,theElements,adjacencyList,theDebuggingTextAreaID,"Content Count"); //contents, degree - modes
 ...
 function editLevel()
 {
	...
	for(var j=0;j<elements.length;j++)	// nullify the initial angles of the elements
	{
		elements[j].angle = undefined;
	}
	ctx.clearRect(0,0,canvas.width,canvas.height);
	ctx1.clearRect(0,0,canvas1.width,canvas1.height);
	ctx2.clearRect(0,0,canvas2.width,canvas2.height);
	
	drawTheChart("myCanvas","MyCanvas2",levelRanges,elements,adjacencyList,"debugTA","Content Count"); //contents, degree - modes
	...
	for(var i=0;i<modes.options.length;i++)
	{
		...
		if(modes.options[i].selected == true && theMode!=modes.options[i].value)
		{
			...
			if(theMode == "Degree")
			{
				...
				lRangesBox.size = 2;
				drawTheChart("myCanvas","MyCanvas2",levelRanges,elements,adjacencyList,"debugTA","Degree"); //contents, degree - modes
			}
...
function viewDetails()
{
	...
	for(var i=0;i<theSelectedList.options.length;i++)
	{
		...
		{
			details += "-----------------------\n";
			details += "Regulon Name: " + elements[theSelectedList.options[i].ind].name +  "\n";
			details += "Description: " + elements[theSelectedList.options[i].ind].description +  "\n";
			details += "Genes contained: " + elements[theSelectedList.options[i].ind].contentCount +  "\n";
			...
			{
				details += ""+ elements[neighbors[j]].name;
...
function selectionAction(optn,ctrl)
{
   ...
   if(!ctrl)
   {
		...
		newoptn.ind = optn.ind; // corresponds to the index in the alllist and the elements array
...
function addOption(selectbox)
{
	...
	{
		clearInterval(stID);
		ta.value = "Loaded " + elements.length + " Regulons";
...
{
	stID = setInterval("addOption('allList')", 1);
}
...
function removeSelected()
{
	...
	for(var i=0;i<sbox.options.length;i++)
	{
		if(sbox.options[i].selected) 
		{
			...
			drawCircle(elements[sbox.options[i].ind].X,elements[sbox.options[i].ind].Y,"#33FF00");
...
$(function () {
    ...
    function getData() {
        ...
        for (var i = 0; i < elements.length; ++i) {
            d.push([i, elements[i].contentCount]);
        }
		...
		if (item) {
			if (previousPoint != item.datapoint) {
				...
				showTooltip(item.pageX, item.pageY,
							item.series.label + " in " + elements[x].name + " is " + y);
			}
		}
		else {
			$("#tooltip").remove();
...
function fullscreen1()
{
		...
		var otherZoomIn = document.getElementById('zoomin2');
		var otherZoomOut = document.getElementById('zoomout2');
		var otherFullScreen = document.getElementById('fullScreen2');
...
<body onload="init()">
	...
	<div id="loading" style="display:none;">
		...
		<div id="loadedValue" style="position:absolute;top:50%;left:45%;z-index:5001;background-color:white;border: 2px solid gray"><center>&nbsp; Loading : 0% &nbsp;</center></div>
	</div>
	...
	<div>
		...
		<div>
			...
			<div id="zoom_panel2">
				<button type="submit" id="zoomin2"     style="z-index: 4;position:absolute;top:10px;left:10px;height:30px;"><img src = "zoom_in.png" width = "20px" height = "20px"></button>
				<button type="submit" id="zoomout2"    style="z-index: 4;position:absolute;top:10px;left:50px;height:30px;"><img src="zoom_out.png" width = "20px" height = "20px"></button>
				<button type="submit" id="fullScreen2" style="z-index: 4;position:absolute;top:10px;left:90px;height:30px;" onclick="fullscreen2();"><img src="fullscreen.png" width = "20px" height = "20px" ></button>
	...
	<div id = "debug">
		...
		<!-- Level Editing Expandable Content box start -->
		<div style="border: 1px solid #000; padding: 0px; background: #EEEEEE; ">
			<table border="0" cellspacing="0" cellpadding="2" width="100%" style="background: #000000; color: #FFFFFF; ">
				<tr>
					<td>Level Editing</td>
		...
		<!-- Level Editing Expandable Content box end  -->
